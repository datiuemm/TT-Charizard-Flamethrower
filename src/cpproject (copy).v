/*
 * Project: Zizard Only (Fixed State)
 * Features: Rain effect, Fire breathing logic, Fixed Ground/Sky
 */
...
module tt_um_datdt_charizard(
    ...
    // --- ROMS & PALETTE (Only Zizard and Fire) ---
    reg [2:0] rom_zizard[0:16383], rom_fire[0:16383];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (pix_x == 0 && pix_y == 0) begin
                // Bấm giữ Up + Down để khè lửa
                is_firing <= (g_up && g_down);
            ...
            if (video_active) begin
                if (idx != 0) begin
                    ...
                    // Nền: Trời & Cỏ
                    if (pix_y < 350) begin
                        R <= 2'b00; G <= 2'b00; B <= 2'b01; // Trời tím
                    end else begin
                        R <= 2'b00; G <= 2'b01; B <= 2'b00; // Cỏ xanh
                    end
                end
            end else begin
                {R, G, B} <= 6'b000000;
...
// Module Gamepad giữ nguyên để nhận tín hiệu điều khiển
